...
static int
send_func(seL4_Word endpoint, seL4_Word seed, seL4_Word arg2, seL4_Word arg3)
{
...
static int
nbsend_func(seL4_Word endpoint, seL4_Word seed, seL4_Word arg2, seL4_Word arg3)
{
    FOR_EACH_LENGTH(length) {
        seL4_MessageInfo_t tag = seL4_MessageInfo_new(0, 0, 0, length);
        for (int i = 0; i < length; i++) {
...
static int
call_func(seL4_Word endpoint, seL4_Word seed, seL4_Word arg2, seL4_Word arg3)
{
    FOR_EACH_LENGTH(length) {
        seL4_MessageInfo_t tag = seL4_MessageInfo_new(0, 0, 0, length);
...
static int
wait_func(seL4_Word endpoint, seL4_Word seed, seL4_Word arg2, seL4_Word arg3)
{
    FOR_EACH_LENGTH(length) {
...
static int
nbwait_func(seL4_Word endpoint, seL4_Word seed, seL4_Word unused, seL4_Word nbwait_should_wait)
{
...
static int
replywait_func(seL4_Word endpoint, seL4_Word seed, seL4_CPtr reply, seL4_Word unused)
{
    int first = 1;
    ...
    FOR_EACH_LENGTH(length) {
        ...
        if (first) {
            ...
            tag = seL4_ReplyRecv(endpoint, tag, &sender_badge, reply);
        }
...
reply_and_wait_func(seL4_Word endpoint, seL4_Word seed, seL4_CPtr reply, seL4_Word unused)
{
    ...
    FOR_EACH_LENGTH(length) {
        ...
        tag = seL4_Recv(endpoint, &sender_badge, reply);
...
{
    FOR_EACH_LENGTH(length) {
        seL4_NBSendRecv(endpoint, seL4_MessageInfo_new(0, 0, 0, MAX_LENGTH), endpoint, NULL, reply);
    }
...
DEFINE_TEST(IPC0003, "Test seL4_Send + seL4_Send(reply) + seL4_Recv", test_call_reply_and_wait)
...
DEFINE_TEST(IPC1003, "Test inter-AS seL4_Send + seL4_Send(reply) + seL4_Recv", test_call_reply_and_wait_interas)
...
single_client_server_chain_test(env_t env, int fastpath, int prio_diff)
{
    ...
    for (int i = 0; i < num_proxies; i++) {
        ...
        int error = seL4_SchedContext_Unbind(proxies[i].thread.sched_context.cptr);
...
ipc0016_reply_once_fn(seL4_CPtr endpoint, seL4_CPtr reply)
{
    ...
    seL4_NBSendRecv(endpoint, info, endpoint, NULL, reply);
...
static int test_fault_handler_donated_sc(env_t env)
{
    ...
    error = seL4_TCB_SetSpace(faulter.thread.tcb.cptr, endpoint,
...
ipc22_server_fn(seL4_CPtr init_ep, seL4_CPtr reply_cap)
{
    ...
    seL4_NBSendRecv(init_ep, info, init_ep, NULL, reply_cap);
...
ipc22_stack_spawner_fn(env_t env, seL4_CPtr endpoint, int server_prio, seL4_Word unused)
{
    ...
    for (int i = 0; i < RUNS; i++) {
        ...
        seL4_NBSendRecv(first_ep, seL4_MessageInfo_new(0, 0, 0, 0), endpoint, NULL,
    ...
    seL4_NBSendRecv(first_ep, seL4_MessageInfo_new(0, 0, 0, 0), endpoint, NULL, servers[RUNS - 1].thread.reply.cptr);
}
...
static int test_stack_spawning_server(env_t env)
{
    ...
    int error = seL4_SchedContext_Unbind(stack_spawner.thread.sched_context.cptr);
    test_eq(error, seL4_NoError);
...
static void ipc23_server_fn(seL4_CPtr client_ep, seL4_CPtr wait_ep, seL4_CPtr reply)
{
    ...
    seL4_NBSendRecv(wait_ep, seL4_MessageInfo_new(0, 0, 0, 0), client_ep, NULL, reply);
