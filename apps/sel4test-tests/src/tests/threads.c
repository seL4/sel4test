...
int test_tcb_null_cspace_configure(env_t env)
{
    ...
    error = seL4_TCB_Configure(thread.thread.tcb.cptr, 0, seL4_CapNull,
                               seL4_Prio_new(100, 100, seL4_MinCrit, seL4_MinCrit), seL4_CapNull, seL4_CapNull, 
...
int test_tcb_null_cspace_setspace(env_t env)
{
    ...
    /* This should fail because we're passing an invalid CSpace cap. */
    error = seL4_TCB_SetSpace(thread.thread.tcb.cptr, 0, seL4_CapNull, seL4_CapNull,
                              seL4_CapData_Guard_new(0, 0), env->page_directory,
