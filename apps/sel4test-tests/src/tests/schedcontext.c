...
test_sched_control_configure(env_t env)
{
    ...
    /* test it works */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 5000llu, 5000llu, 0);
    ...
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), tcb, 5000llu, 5000llu, 0);
    test_eq(error, seL4_InvalidCapability);

    /* test a 0 budget doesn't work */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 0llu, 5000llu, 0);
    test_eq(error, seL4_RangeError);

    /* test a period of 0 doesn't work */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 0llu, 0llu, 0);
    test_eq(error, seL4_RangeError);

    /* test budget > period doesn't work */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 5000llu, 1000llu, 0);
    test_eq(error, seL4_RangeError);
...
test_sched_control_reconfigure(env_t env)
{
    ...
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 10000llu, 10000llu, 0);
    test_eq(error, seL4_NoError);
    ...
    /* less */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&env->simple, 0), sc, 3000llu, 3000llu, 0);
    test_eq(error, seL4_NoError);
...
test_delete_tcb_on_notification_context(env_t env)
{
    ...
    int error = seL4_SchedContext_Unbind(one.thread.sched_context.cptr);
    test_eq(error, seL4_NoError);
...
{
    ZF_LOGD("Client send\n");
    seL4_NBSendWait(send_ep, seL4_MessageInfo_new(0, 0, 0, 0), wait_ep, NULL);
}
...
sched_context0008_server_fn(seL4_CPtr init_ep, seL4_CPtr wait_ep)
{
    ...
    seL4_NBSendWait(init_ep, seL4_MessageInfo_new(0, 0, 0, 0), wait_ep, NULL);
    ZF_LOGD("Server exit\n");
...
test_delete_sendwait_tcb(env_t env)
{
    ...
    error = seL4_SchedContext_Bind(client.thread.sched_context.cptr, client.thread.tcb.cptr);
    test_eq(error, seL4_NoError);
...
{
    seL4_NBSendWait(ep, seL4_MessageInfo_new(0, 0, 0, 0), ep, NULL);
}
...
test_sched_context_unbind_server(env_t env)
{
    ...
    int error = seL4_SchedContext_UnbindObject(client.thread.sched_context.cptr, server.thread.tcb.cptr);
    test_eq(error, seL4_NoError);
...
{
    ZF_LOGD("Proxy init\n");
    seL4_NBSendRecv(in, seL4_MessageInfo_new(0, 0, 0, 0), in, NULL, reply);
